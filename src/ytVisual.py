...
class App(ttk.Window):
    def __init__(self, title, size, font):
        ...
        choose_folder_button = ttk.Button(output_frame, text='Choose Folder', command=self.choose_folder,image=choose_folder_icoTk)
        ...
        self.download_button = ttk.Button(downl_frame, text='Download', bootstyle='success',
                                          command=self.download_button_clicked, state='enabled',image=download_gifTk,compound='left')
    ...
    def download_button_clicked(self):
        ...
        def download_proccess():
            try:
                video_filename, video_ext, audio_codec = ytDownloaderFunctions.download_video(video_url, chosen_format,
                                                                                              self.output_folder,
                                                                                              self.progress_bar)
                ...
                if audio_codec == 'none':
                    ...
                    output_filename = os.path.join(self.output_folder.get(), 'video_merged.mp4')
